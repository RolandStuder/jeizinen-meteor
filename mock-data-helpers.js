...
random = {
	name: function() {
		return random.firstName() + " " + random.familyName();
	},
	firstName: function() {
	...
	email: function() {
		return random.firstName() + "." + random.familyName() + "@" + random.pick({options: 'example,gmail,yahoo,bigcorp'}) + ".com";
	},
	pick: function(hash) { // takes a comma separated string as argument
		if(hash.options) {
			return sample(hash.options.split(","));
		}
	}, 
